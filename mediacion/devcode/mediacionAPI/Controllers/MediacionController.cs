...
namespace mediacionAPI.Controllers
{
    ...
    public class MediacionController : Controller
    {
        ...
        //Nethereum.Web3.Web3 geth;  //Kaleido
        ...
        public async Task<IActionResult> Post(int idMediacion, [FromBody]JArray documentos)
        {
            try 
            {
                ...
                foreach (JObject item in documentos)
                {
                    ...
                    var transactionHash = await creaNuevaMediacionFunction.SendTransactionAsync(mediadorAddress, new HexBigInteger(900000), null, descripcion, idMediacion, ipfsHash);
        ...
        public async Task<IActionResult> Put(int idMediacion, [FromBody]JArray documentos)
        {
            try
            {
                ...
                foreach (JObject item in documentos)
                {
                    ...
                    var transactionHash = await agregaDocumentoFunction.SendTransactionAsync(mediadorAddress, new HexBigInteger(900000), null, descripcion, idMediacion, ipfsHash);
        ...
        private async Task VerificaExistenciaSmartContract() 
        {
            if (contractAddress == string.Empty)
            {
                ...
                try
                {
                    ...
                    /************* Parece ser que no es necesario la siguientelines ni en Clique ni en Kaleido ***********/
