...
namespace mediacionAPI.Controllers
{
    ...
    public class TestController : Controller
    {
        ...
        string bytecode = "";
        int multiplier = 7;
        Web3Geth geth;
        ...
        public async Task<IActionResult> Post()
        {
            ...
            var transactionHash = await creaNuevaMediacionFunction.SendTransactionAsync(senderAddress, new HexBigInteger(900000), null, "123qwe456asd", cjaAddress);
            ...
            var result0 = await mediacionStruct.CallDeserializingToObjectAsync<Mediacion>(senderAddress, 0);
            var result1 = await mediacionStruct.CallDeserializingToObjectAsync<Mediacion>(senderAddress, 1);
        ...
        private async Task VerificaExistenciaSmartContract() 
        {
            if (contractAddress == string.Empty)
            {
                ...
                try
                {
                    var transactionHash = await geth.Eth.DeployContract.SendRequestAsync(abi, bytecode, senderAddress, new Nethereum.Hex.HexTypes.HexBigInteger(900000));
                    var receipt = await geth.Eth.Transactions.GetTransactionReceipt.SendRequestAsync(transactionHash);
                    while (receipt == null)
                    {
                        Thread.Sleep(5000);
